...
export class ProfileViewPage implements OnInit {
 ...
 // @ViewChild('popover') popover;
  ...
  async fileDownload1(filePath: string, fileName: string, fileType: string) {
    if (!filePath) { this.storageservice.warningToast('Error: No file path.'); return; }
    const uniqueFileName = `TalentChek_QR_${new Date().getTime()}.${fileType}`;
    const fullUrl = this.storageservice.baseURL + filePath;
    try {
        this.storageservice.showLoading();
        const result = await Filesystem.downloadFile({
            path: uniqueFileName,
            url: fullUrl,
            directory: Directory.External, // Use External for your plugin version
        });
        this.storageservice.dismissLoading();
        this.storageservice.warningToast('QR Downloaded Successfully');
        this.fileOpener.showOpenWithDialog(result.path, this.getMIMEtype(fileType));
    } catch (error) {
        this.storageservice.dismissLoading();
...
async fileDownloadOffline(filePath: string, fileName: string, fileType: string) {
    if (!filePath) { this.storageservice.warningToast('Error: No file path.'); return; }
    const uniqueFileName = `TalentChek_OfflineQR_${new Date().getTime()}.${fileType}`;
    const fullUrl = this.storageservice.baseURL + filePath;
    try {
        this.storageservice.showLoading();
        const result = await Filesystem.downloadFile({
        ...
        this.storageservice.dismissLoading();
        this.storageservice.warningToast('Offline QR Downloaded Successfully');
        this.fileOpener.showOpenWithDialog(result.path, this.getMIMEtype(fileType));
    } catch (error) {
        this.storageservice.dismissLoading();
        this.storageservice.warningToast('Error in download.');

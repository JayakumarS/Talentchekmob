...
export class OrgProfilePage implements OnInit {
  ...
  constructor(private fb: FormBuilder, public storageservice: StorageService, public modalController: ModalController, public alertController: AlertController,
    //private camera: Camera,
     public router: Router, private ngZone: NgZone, private toastController: ToastController, private route: ActivatedRoute, private languageService: LanguageService, private scroller: ViewportScroller) {
  ...
  try {
    ...
    const imagePathServiceUrl = "api/auth/app/CommonUtility/uploadImagePath";
    this.storageservice.postrequest(imagePathServiceUrl, postData).subscribe(result => {
      if (result && result['success'] === true) {
        this.docForm.patchValue({
          'orgLogo': result['uploadPhotoPath'],
        });
        console.log('Updated orgLogo path:', result['uploadPhotoPath']);
      }
    });

  } catch (error) {
    console.log("User cancelled gallery selection", error);
  }
}
